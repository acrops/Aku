...
int main()
{
	vector<int> x;
	int temp;
	cout << "输入任意整数，系统将进行排序" << endl<<"输入A进行升序排列"<<endl<<"输入D进行降序排列"<<endl;
	char ans = 'A';
	cin.get(ans);
	if(ans!='A'&&ans!='D') cout << "错误的输入，默认进行升序" << endl;
	...
	if(ans=='D')	sort(x.begin(), x.end(), comp);//降序排列
	else	sort(x.begin(), x.end());//默认升序
	show(x, p);
	return 0;
}